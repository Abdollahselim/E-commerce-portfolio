...
export function ProcessSection() {
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-px overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-white/10 md:grid-cols-4">
        {processSteps.map((step, index) => {
          const Icon = step.icon;
          return (
            <MotionReveal key={step.title.en} delay={index * 0.06}>
              <article
                className="process-card-hover h-full bg-[var(--bg-card)] p-6 transition-colors"
              >
                <div className="mb-8 flex items-center justify-between">
                  <span className="font-display text-3xl text-[var(--text-3)]">
                    0{index + 1}
                  </span>
                  <Icon
                    className="process-card-icon h-5 w-5 text-[var(--gold)]"
                    aria-hidden="true"
                  />
                </div>
